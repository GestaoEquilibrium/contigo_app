      ...
      <aside className="lateral">
        ...
        {atual && (
          <div className="empresa">
            ...
            <span className="gfraco">{NOME_PAPEL[atual.papel]}{demo && ' · demonstração'}</span>
...
export function Cabeca({ titulo, sub, children }: { titulo: string; sub?: ReactNode; children?: ReactNode }) {
  return (
    <div className="cabeca">
      <div><h1>{titulo}</h1>{sub && <p className="gfraco">{sub}</p>}</div>
...
export function Carregando() { return <div className="gcarregando">Um instante…</div>; }
...
export function BotaoCopiar({ texto, rotulo = 'Copiar' }: { texto: string; rotulo?: string }) {
  ...
  return (
    <button className="gbtn gbtn-leve" onClick={async () => { try { await navigator.clipboard.writeText(texto); setFeito(true); setTimeout(() => setFeito(false), 1800); } catch { /* sem clipboard */ } }}>
