  ...
  return (
    <>
      <Cabeca titulo="Empresas e contratos" sub="A parte do Contigo: quem é cliente, quantas licenças, e quem administra o portal de cada empresa.">
        <button className="gbtn gbtn-coral" onClick={() => { setAlvo(null); setModal('empresa'); }}><Ico.mais />Nova empresa</button>
      </Cabeca>
      <Erro msg={erroAcao} /><Aviso msg={aviso} />

      <div className="gcartao" style={{ marginBottom: 14 }}>
        <h3>Agregação NR-1</h3>
        <p className="gfraco gpequeno">Roda sozinha no dia 1 de cada mês. Aqui você pode recalcular um mês — útil para ver o painel logo depois de os primeiros check-ins entrarem.</p>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <input className="gcampo" type="month" style={{ width: 'auto' }} value={periodo.slice(0, 7)} onChange={e => setPeriodo(e.target.value + '-01')} />
          <button className="gbtn gbtn-leve" onClick={() => setPeriodo(primeiroDiaDoMes())}>Este mês</button>
          <button className="gbtn gbtn-coral" onClick={calcular} disabled={calculando}><Ico.atualizar />{calculando ? 'Calculando…' : 'Calcular agora'}</button>
        </div>
      </div>
...
function CartaoEmpresa({ empresa, abrirContrato, abrirAcesso, entrar, aoMudar }: { empresa: Empresa; abrirContrato: () => void; abrirAcesso: () => void; entrar: () => void; aoMudar: () => void }) {
  ...
  return (
    <div className="gcartao">
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'flex-start' }}>
        <div>
          <h3 style={{ fontSize: 18 }}>{empresa.nome} {!empresa.ativa && <span className="gselo inativo">inativa</span>}</h3>
          <p className="gpequeno gfraco" style={{ margin: 0 }}>{empresa.cnpj ?? 'sem CNPJ'}</p>
        </div>
        <div className="acoes">
          <button className="gbtn gbtn-leve" onClick={entrar}>Abrir como esta empresa</button>
          <button className="gbtn gbtn-leve" onClick={abrirAcesso}><Ico.chave />Liberar acesso</button>
          <button className="gbtn gbtn-leve" onClick={abrirContrato}><Ico.contrato />Novo contrato</button>
          <button className="gbtn gbtn-texto" onClick={alternar}>{empresa.ativa ? 'Inativar' : 'Reativar'}</button>
        </div>
      </div>
      <div className="grade" style={{ marginTop: 12 }}>
        <div className="gcartao gsuave">
          <b>Contrato</b>
          {vigente ? <p className="gpequeno" style={{ margin: '4px 0 0' }}>{vigente.licencas} licenças · {dataBr(vigente.vigenciaInicio)} → {dataBr(vigente.vigenciaFim)} · {vigente.camada2 ? 'com' : 'sem'} Vamos conversar
            <br /><button className="gbtn gbtn-texto" style={{ padding: 0 }} onClick={() => encerrar(vigente)}>Encerrar</button></p>
            : <p className="gpequeno gfraco" style={{ margin: '4px 0 0' }}>Sem contrato vigente — a empresa não consegue convidar.</p>}
        </div>
        <div className="gcartao gsuave"><b>Uso</b><p className="gpequeno" style={{ margin: '4px 0 0' }}>{dado?.uso ? `${dado.uso.ativos} ativos · ${dado.uso.convidados} convidados` : '—'}</p></div>
        <div className="gcartao gsuave"><b>Acessos ao portal</b><p className="gpequeno" style={{ margin: '4px 0 0' }}>{dado?.acessos.length ? dado.acessos.filter(a => a.ativo).map(a => `${a.email} (${a.papel})`).join(', ') : 'ninguém ainda'}</p></div>
      </div>
    </div>
  );
}
  ...
  return (
    <Modal aberto={aberto} fechar={fechar} titulo="Nova empresa cliente">
      <form onSubmit={enviar}>
        <label className="rotulo">Razão social ou nome</label><input className="gcampo" value={nome} onChange={e => setNome(e.target.value)} autoFocus />
        <label className="rotulo">CNPJ (opcional)</label><input className="gcampo" value={cnpj} onChange={e => setCnpj(e.target.value)} placeholder="00.000.000/0001-00" />
        <div style={{ marginTop: 10 }}><Erro msg={erro} /></div>
        <div className="gpe"><button type="button" className="gbtn gbtn-texto" onClick={fechar}>Cancelar</button><button className="gbtn gbtn-coral" type="submit">Criar</button></div>
      </form>
    </Modal>
  );
}
    ...
    <Modal aberto={aberto} fechar={fechar} titulo={`Novo contrato — ${empresa.nome}`}>
      <form onSubmit={enviar}>
        <div className="linha-form">
          <div><label className="rotulo">Licenças</label><input className="gcampo" type="number" min={1} value={licencas} onChange={e => setLicencas(Number(e.target.value))} /></div>
          <div><label className="rotulo">Início</label><input className="gcampo" type="date" value={ini} onChange={e => setIni(e.target.value)} /></div>
          <div><label className="rotulo">Fim</label><input className="gcampo" type="date" value={fimS} onChange={e => setFim(e.target.value)} /></div>
        </div>
        ...
        <div style={{ marginTop: 10 }}><Erro msg={erro} /></div>
        <div className="gpe"><button type="button" className="gbtn gbtn-texto" onClick={fechar}>Cancelar</button><button className="gbtn gbtn-coral" type="submit">Criar contrato</button></div>
      </form>
    </Modal>
  );
}
...
function NovoAcesso({ aberto, fechar, empresa, aoCriar }: { aberto: boolean; fechar: () => void; empresa: Empresa; aoCriar: (email: string) => void }) {
  ...
  return (
    <Modal aberto={aberto} fechar={fechar} titulo={`Liberar acesso — ${empresa.nome}`}>
      <form onSubmit={enviar}>
        <p className="gfraco gpequeno">A pessoa precisa ter criado a conta no portal ("Criar conta" na entrada). Se ainda não criou, o banco avisa.</p>
        <label className="rotulo">E-mail</label><input className="gcampo" type="email" value={email} onChange={e => setEmail(e.target.value)} autoFocus />
        ...
        <div style={{ marginTop: 10 }}><Erro msg={erro} /></div>
        <div className="gpe"><button type="button" className="gbtn gbtn-texto" onClick={fechar}>Cancelar</button><button className="gbtn gbtn-coral" type="submit">Liberar</button></div>
      </form>
    </Modal>
  );
}
