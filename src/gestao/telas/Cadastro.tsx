  ...
  return (
    <>
      ...
      <form onSubmit={criar} style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
        <input className="gcampo" placeholder="Nome do novo setor" value={novo} onChange={e => setNovo(e.target.value)} />
        <button className="gbtn gbtn-coral" type="submit"><Ico.mais />Criar</button>
      </form>
          ...
          <tbody>
            {dado!.setores.map(s => { const n = conta(s.id); return (
              <tr key={s.id}>
                ...
                <td className="num">{n >= 12 ? <span className="gselo ativo">aparece</span> : <span className="gselo">precisa de {12 - n} a mais</span>}</td>
                <td><div className="acoes-linha"><button className="gbtn gbtn-texto" onClick={() => renomear(s.id, s.nome)}>Renomear</button><button className="gbtn gbtn-texto" onClick={() => apagar(s.id, s.nome, n)}>Apagar</button></div></td>
              </tr>
            ); })}
          </tbody>
        </table>
  ...
  return (
    <>
      ...
      <div className="grade" style={{ marginBottom: 14 }}>
        {PAPEIS.map(p => <div className="gcartao gsuave" key={p.valor}><b>{p.nome}</b><p className="gpequeno gfraco" style={{ margin: '4px 0 0' }}>{p.descricao}</p></div>)}
      </div>
        ...
        <table className="tabela">
          ...
          <tbody>
            {dado!.map(a => { const souEu = a.email === meus?.email; return (
              <tr key={a.usuarioId}>
                <td><b>{a.email}</b>{souEu && <span className="gpequeno gfraco"> (você)</span>}</td>
                <td>
                  <select className="gcampo" style={{ minHeight: 32, padding: '4px 8px', width: 'auto' }} value={a.papel} onChange={e => mudarPapel(a.usuarioId, e.target.value as Papel)} disabled={souEu || !a.ativo}>
                ...
                <td><div className="acoes-linha">{!souEu && <button className="gbtn gbtn-texto" onClick={() => alternar(a.usuarioId, a.ativo, a.email)}>{a.ativo ? 'Remover' : 'Reativar'}</button>}</div></td>
              </tr>
            ); })}
          </tbody>
        </table>
  ...
  return (
    <>
      ...
      {u.licencas == null ? <Vazio>Nenhum contrato vigente. Sem contrato, a empresa não consegue convidar ninguém — fale com a equipe do Contigo.</Vazio> : (
        <div className="grade">
          <div className="gcartao"><h3>Licenças</h3><div className="numero">{ocupadas}<small>de {u.licencas} em uso</small></div><p className="gpequeno gfraco">{u.ativos} pessoas já entraram · {u.convidados} convidadas ainda não</p></div>
          <div className="gcartao"><h3>Vigência</h3><div className="numero" style={{ fontSize: 20 }}>{dataBr(u.vigenciaInicio)} → {dataBr(u.vigenciaFim)}</div></div>
          <div className="gcartao"><h3>Camadas</h3>
            <p><span className="gselo ativo">Prevenção diária</span> <span className="gpequeno gfraco">ritual, trilhas, rastreio</span></p>
            <p><span className={'gselo ' + (u.camada2 ? 'ativo' : '')}>Vamos conversar</span> <span className="gpequeno gfraco">{u.camada2 ? 'contratada — abre quando a equipe estiver dimensionada' : 'não contratada'}</span></p>
            <p style={{ margin: 0 }}><span className={'gselo ' + (u.camada3 ? 'ativo' : '')}>Pronto atendimento</span> <span className="gpequeno gfraco">{u.camada3 ? 'contratada' : 'não contratada'}</span></p>
          </div>
        </div>
      )}
      <div className="gcartao gdestaque" style={{ marginTop: 14 }}>
    ...
    <>
      ...
      {dado!.length === 0 ? <Vazio>Nada registrado ainda.</Vazio> : (
        <table className="tabela">
          ...
          <tbody>{dado!.map(a => <tr key={a.id}><td className="gpequeno">{dataBr(a.em, true)}</td><td>{a.email ?? <span className="gfraco">sistema</span>}</td><td>{NOME_OP[a.operacao] ?? a.operacao} em <b>{NOME_TABELA[a.tabela] ?? a.tabela}</b></td><td className="gpequeno gfraco">{resumo(a)}</td></tr>)}</tbody>
        </table>
      )}
    </>
  );
