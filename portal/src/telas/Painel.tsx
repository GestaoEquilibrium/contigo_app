  ...
  return (
    <>
      <Cabeca titulo="Painel NR-1" sub={<>Só o grupo, nunca a pessoa. Recorte com menos de 12 respondentes não aparece — em lugar nenhum.</>}>
        <select className="campo" style={{ width: 'auto' }} value={per} onChange={e => setPeriodo(e.target.value)} aria-label="Período">
          {periodos.map(p => <option key={p} value={p}>{mesBr(p)}</option>)}
        </select>
        <button className="btn btn-leve" onClick={exportar}><Ico.baixar />Exportar para o PGR (CSV)</button>
      </Cabeca>
      ...
      <div className="grade">
        {indicadores.map(i => {
          ...
          return (
            <div className="cartao" key={i.codigo}>
              <h3>{i.nome}</h3>
              {l ? (
                <>
                  <div className="numero">{formatarValor(i.codigo, i.origem, l.valor)}<small>{l.n} pessoas</small></div>
                  {ant && <p className="pequeno fraco" style={{ margin: '4px 0 0' }}>Mês anterior: {formatarValor(i.codigo, i.origem, ant.valor)}</p>}
                </>
              ) : <p className="fraco">Recorte pequeno demais.</p>}
              <p className="pequeno fraco" style={{ margin: '6px 0 0' }}>{i.descricao}</p>
            </div>
          );
        })}
      </div>
      ...
      <p className="pequeno fraco" style={{ marginTop: 10 }}>
