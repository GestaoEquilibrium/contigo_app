  ...
  const enviar = async (e: FormEvent) => {
    ...
    try {
      if (modo === 'senha') {
        ...
        await dados.enviarCodigo(em, `${window.location.origin}/`);
  ...
  return (
    <div className="entrada">
      <form className="caixa" onSubmit={enviar}>
        <div className="marca"><i />Contigo <span className="fraco" style={{ fontWeight: 400 }}>· Gestão</span></div>
        <p className="fraco">Indicadores agregados da sua empresa. Nenhum dado individual chega aqui — nem por engano.</p>
        {demo && <span className="selo demo">Demonstração · dados inventados</span>}
        ...
        <input className="campo" type="email" autoComplete="username" value={email} onChange={e => setEmail(e.target.value)} placeholder="nome@empresa.com.br" autoFocus />
        {(modo === 'senha' || modo === 'criar') && (
          <>
            <label className="rotulo">{modo === 'criar' ? 'Escolha uma senha' : 'Senha'}</label>
            <input className="campo" type="password" autoComplete={modo === 'criar' ? 'new-password' : 'current-password'} value={senha} onChange={e => setSenha(e.target.value)} />
          </>
        )}
        {modo === 'codigo' && codigoEnviado && (
          <>
            <label className="rotulo">Código de seis números</label>
            <input className="campo" inputMode="numeric" autoComplete="one-time-code" maxLength={6} value={codigo} onChange={e => setCodigo(e.target.value)} />
          </>
        )}
        <div style={{ marginTop: 12 }}><Erro msg={erro} /><Aviso msg={aviso} /></div>
        <button className="btn btn-coral" type="submit" disabled={ocupado}>
          {ocupado ? 'Um instante…' : modo === 'senha' ? 'Entrar' : modo === 'criar' ? 'Criar conta' : codigoEnviado ? 'Entrar' : 'Mandar o código'}
        </button>
        {modo === 'criar' && <p className="pequeno fraco" style={{ marginTop: 12 }}>Depois de criar a conta, o administrador da sua empresa (ou a equipe do Contigo) libera o seu acesso.</p>}
      </form>
    </div>
  );
}
  ...
  return (
    <div className="entrada">
      <div className="caixa">
        ...
        <button className="btn btn-leve" onClick={() => dados.sair()}>Sair</button>
      </div>
    </div>
  );
}
